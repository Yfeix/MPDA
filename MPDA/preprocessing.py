...
class Preprocessing:
    ...
    @staticmethod
    def train_models(root, threshold, net):
        cwes = get_cwe(root)
        for cwe in cwes:
            if len(cwes[cwe]) >= threshold:
                Preprocessing._train_model(str(cwe), cwes[cwe], net)
    ...
    @staticmethod
    def _train_model(cwe_name, cwe_paths, net):
        print(cwe_name)
        df = [["input", "label"]]
        ...
        MPDAmodel.train(dataframe, os.path.realpath(__file__)[:-16] + "result/checkpoints/" + cwe_name + ".h5", net)
        filename = os.path.realpath(__file__)[:-16] + '/result/vocab.csv'
